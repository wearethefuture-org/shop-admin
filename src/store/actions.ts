...
import {
  ...
  REQUEST_USERS,
  SWITCH_DARK_MODE,
  REQUEST_UPLOAD_PREWIES_PRODUCT,
  UPLOAD_PREWIES_PRODUCT
} from './types';
...
export const fetchUploadImagesProduct = (formData: FormData, id: number): IActions => ({
  type: REQUEST_UPLOAD_IMAGES_PRODUCT,
  data: { formData, id },
});

export const fetchUploadPrewiesProduct = (productId: number, imgName: string): IActions => ({
  type: REQUEST_UPLOAD_PREWIES_PRODUCT,
  data: { productId, imgName },
});

export const uploadPrewiesProduct = (data: Array<any>): IActions => ({
  type: UPLOAD_PREWIES_PRODUCT,
  data: data,
});
