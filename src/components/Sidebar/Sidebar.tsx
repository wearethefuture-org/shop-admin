...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onSidebarToggle }) => {
  const screenSize: boolean = useMediaQuery((theme: Theme) =>
  ...
  const sidebarItems = [
    {
      pageURL: "/dashboard",
      itemIcon: <HouseIcon fontSize="small" className={scss.icon} />,
    },
    {
      pageURL: "/categories",
      itemIcon: <CategoryIcon fontSize="small" className={scss.icon} />,
    },
    {
      pageURL: "/products",
      itemIcon: <ShoppingCartIcon fontSize="small" className={scss.icon} />,
    },
    {
      pageURL: "/statistic",
      itemIcon: <EqualizerIcon fontSize="small" className={scss.icon} />,
    },
    {
      pageURL: "/users",
      itemIcon: <GroupIcon fontSize="small" className={scss.icon} />,
    },
  ];
        ...
        <List className={classes.mainNav}>
          {["Invoice", "Settings"].map((text, index) => (
            ...
            >
              <ListItemIcon className={classes.itemIcon}>
                {index % 2 === 0 ? (
                  <ReceiptIcon fontSize="small" className={scss.icon} />
                ) : (
                  <SettingsIcon fontSize="small" className={scss.icon} />
                )}
              </ListItemIcon>
